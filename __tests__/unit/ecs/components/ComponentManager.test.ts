...
describe('ComponentManager', () => {
    ...
    describe('getEntityComponents', () => {
        it('should return all components for an entity', () => {
            const testComponent = new TestComponent(100);
            const anotherTestComponent = new AnotherTestComponent('hello');
            manager.addComponent(entity, TestComponent, testComponent);
            manager.addComponent(entity, AnotherTestComponent, anotherTestComponent);

            const components = manager.getEntityComponents(entity);
            expect(components.length).toEqual(2);
            expect(components).toContainEqual(testComponent);
            expect(components).toContainEqual(anotherTestComponent);
        });

        it('should return an empty array for an entity with no components', () => {
            const components = manager.getEntityComponents(entity);
            expect(components.length).toEqual(0);
        });
    });

    describe('getEntityComponentsType', () => {
        it('should return all component types for an entity', () => {
            manager.addComponent(entity, TestComponent, new TestComponent(100));
            manager.addComponent(entity, AnotherTestComponent, new AnotherTestComponent('hello'));

            const componentTypes = manager.getEntityComponentsType(entity);
            expect(componentTypes.length).toEqual(2);
        ...
        it('should return an empty array for an entity with no components', () => {
            const componentTypes = manager.getEntityComponentsType(entity);
            expect(componentTypes.length).toEqual(0);
        });
    });

    it('should ensure getEntityComponents and getEntityComponentsType return consistent results', () => {
        manager.addComponent(entity, TestComponent, new TestComponent(100));
        manager.addComponent(entity, AnotherTestComponent, new AnotherTestComponent('hello'));
        ...
        expect(componentTypes.length).toEqual(components.length);
        componentTypes.forEach((type, index) => {
            expect(components[index]).toBeInstanceOf(type);
        });
    });
